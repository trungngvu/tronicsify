  ...
  try {
    const promises = req.body.products.map(async (p) => {
      ...
      let originalPrice = Number(
        productDb.subProducts[p.style].sizes.find((x) => x.size == p.size).price
      );
      let quantity = Number(
        productDb.subProducts[p.style].sizes.find((x) => x.size == x.size).qty
      );
      ...
      return {
        ...
        price:
          discount > 0
            ? Number((originalPrice - originalPrice / discount).toFixed(2))
            : originalPrice,
        quantity: quantity,
